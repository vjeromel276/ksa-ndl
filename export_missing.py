...
def main():
    ...
    # 5) Build dict[ticker] → [list of missing dates]
        miss_df = pd.DataFrame(index=missing_idx).reset_index()
        missing_map = (
            miss_df
            .groupby("ticker")["date"]
        ...
        # 6) Write JSON file
        with open("missing_pairs.json", "w") as fp:
            json.dump(missing_map, fp, indent=2)

        print(f"Exported {len(missing_map)} tickers with missing dates to missing_pairs.json")
